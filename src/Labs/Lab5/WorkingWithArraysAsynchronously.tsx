...
export default function WorkingWithArraysAsynchronously() {
  ...
  const deleteTodo = async (todo: any) => {
    try {
        /**
         * wrapping the request in a try/catch clause.
         * if the request fails with HTTP error response, then the body of the try block
         * aborted and body of the catch clause executes instead.
         */
    await client.deleteTodo(todo);
    const newTodos = todos.filter((t) => t.id !== todo.id);  // Optimistic implementation ignore if delete at the server side was successful
    setTodos(newTodos);
    } catch (error: any) {
    console.log(error);
  ...
  return (
    <div id="wd-asynchronous-arrays">
      ...
      <div className="d-flex">
        <h4>Todos</h4>
        <FaPlusCircle onClick={createTodo} className="text-success fs-3 ms-auto me-1" id="wd-create-todo" /> 
