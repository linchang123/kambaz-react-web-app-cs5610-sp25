...
export default function WorkingWithArrays() {
  ...
  return (
    <div id="wd-working-with-arrays">
      ...
      <br /><br /><hr />
      <Form>
        <h4>Updating an Item's completed property</h4>
        <Form.Group as={Row} className="mb-3">
            <Form.Label column sm={2}> Todo Item ID </Form.Label>
            <Col sm={2}>
            <Form.Control placeholder="1" defaultValue={todo.id} onChange={(e) => setTodo({ ...todo, id: e.target.value })} />
            </Col>
            <Col sm={2}>
                <Form.Check type="checkbox" label="completed" className="mt-2"
            ...
            <Col sm={6}>
            <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">Update Todo Completed Status</a>
            </Col>
        </Form.Group>
        <h4>Updating an Item's description property</h4>
        <Form.Group as={Row} className="mb-3">
            <Form.Label column sm={2}> Todo Item ID </Form.Label>
            <Col sm={1}>
            <Form.Control placeholder="1" defaultValue={todo.id} onChange={(e) => setTodo({ ...todo, id: e.target.value })} />
            </Col>
            <Form.Label column sm={2} className="text-end">Todo Item description</Form.Label>
            <Col sm={4}>
                <FormControl defaultValue={todo.description}
            onChange={(e) => setTodo({ ...todo, description: e.target.value })}/>
            </Col>
            <Col sm={3}>
            <a href={`${API}/${todo.id}/description/${todo.description}`} className="btn btn-primary float-end">Update Todo Description</a>
            </Col>
        </Form.Group>
        <hr/>
      </Form>
